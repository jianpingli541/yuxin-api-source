...
```bash
bash scripts/release.sh <镜像标签>          # 例: v1.2.12-yuxin
...
## 运维新增组件（2026-08-15/16 R3 终审批）

以下组件/机制于 R3 终审期间新增，运维时请注意：

| 组件 | 位置 | 说明 |
|---|---|---|
| alert-forwarder | observability/alert-forwarder/（容器 gateway-alert-forwarder） | alertmanager webhook 接收器；告警 JSONL 落 backups/alerts/（随异地备份到 Kali）；外部渠道 webhook 配置在 config/webhooks.list（当前未接，用户决定暂缓） |
| 恢复演练 | scripts/restore-drill.sh | 每月 1 日 05:30 自动演练 pg+CH 还原；日志 backups/auto/drill_*.log |
| 源站锁定 | scripts/cf-origin-lockdown.sh + systemd cf-origin-lockdown.service | 80/443 仅 Cloudflare 官方段可入；勿删 systemd 单元 |
| 异地备份 | Kali 172.252.225.218:/root/backups/feifei-api-gateway/ | Kali systemd user timer 每日 04:10 拉取，14 天保留 |
| admin API token | /root/.config/.secrets/yuxin-admin-access-token.txt (600) | Bearer 认证；用户 lijianping |
| 支付密钥静态加密 | DB options enc1: 前缀（v1.2.10 起） | 读写自动加解密；CRYPTO_SECRET 不可轮换（轮换前需先解密） |
| 登录锁定 | v1.2.11 起 | 用户名+IP 5 次失败锁 15 分钟（redis login_lock:* 键） |
| SSRF 防护 | v1.2.11 起 | 渠道 base_url 禁止内网/回环/云元数据地址 |
