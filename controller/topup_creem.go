...
func verifyCreemSignature(payload string, signature string, secret string) bool {
	if secret == "" {
		logger.LogWarn(context.Background(), fmt.Sprintf("Creem webhook secret 未配置 test_mode=%t signature=%q body_len=%d", setting.CreemTestMode, signature, len(payload)))
		if setting.CreemTestMode {
			logger.LogInfo(context.Background(), fmt.Sprintf("Creem webhook 验签已跳过 reason=test_mode signature=%q body_len=%d", signature, len(payload)))
			return true
		}
		return false
	}
...
func RequestCreemPay(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem 支付请求已收到 user_id=%d body_len=%d", c.GetInt("id"), len(bodyBytes)))
...
func CreemWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem webhook 收到请求 path=%q client_ip=%s signature=%q body_len=%d", c.Request.RequestURI, c.ClientIP(), signature, len(bodyBytes)))
	if signature == "" {
	...
	if !verifyCreemSignature(string(bodyBytes), signature, setting.CreemWebhookSecret) {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Creem webhook 验签失败 path=%q client_ip=%s signature=%q body_len=%d", c.Request.RequestURI, c.ClientIP(), signature, len(bodyBytes)))
		c.AbortWithStatus(http.StatusUnauthorized)
		return
	}
...
func genCreemLink(ctx context.Context, referenceId string, product *CreemProduct, email string, username string) (string, error) {
	...
	logger.LogInfo(ctx, fmt.Sprintf("Creem API 响应已收到 trade_no=%s status_code=%d body_len=%d", referenceId, resp.StatusCode, len(body)))
