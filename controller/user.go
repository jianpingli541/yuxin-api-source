...
func Register(c *gin.Context) {
	...
	// 用户名格式校验: 3-20 位字母/数字/下划线/连字符 (2026-08-18 注册校验加强)
	if !common.ValidateUsername(user.Username) {
		common.ApiErrorI18n(c, i18n.MsgUserInputInvalid, map[string]any{"Error": "username must be 3-20 characters of letters, digits, underscore or hyphen"})
		return
	}
	// 邮箱选填但填了必须是合法格式 (2026-08-18)
	if user.Email != "" && !common.ValidateEmailFormat(user.Email) {
		common.ApiErrorI18n(c, i18n.MsgUserInputInvalid, map[string]any{"Error": "invalid email format"})
		return
	}
	...
	// 密码强度校验: 8-20 位且含大小写字母+数字+特殊符号 (2026-08-18 正式策略, 见 common.ValidatePasswordStrength)
