...
const (
	// PaymentMethodWechat 微信支付支付方式标识
	PaymentMethodWechat = "wechat"
	// PaymentProviderWechat 微信支付服务商标识
	PaymentProviderWechat = "wechat"
)

// RechargeWechat 处理微信支付成功回调的入账。
...
// RefundWechatTopUp 处理微信退款回调的扣减。
// 幂等：已退款直接返回。
func RefundWechatTopUp(tradeNo string, callerIp string) error {
	if tradeNo == "" {
		return errors.New("未提供支付单号")
	}
	...
	if err != nil {
		common.SysError("wechat refund failed: " + err.Error())
		return errors.New("退款失败，请稍后重试")
	}
	if quotaToDeduct > 0 {
		RecordTopupLog(topUp.UserId, fmt.Sprintf("微信支付退款，扣减额度: %v，支付金额: %.2f", logger.FormatQuota(quotaToDeduct), topUp.Money), callerIp, topUp.PaymentMethod, PaymentMethodWechat)
	}
	return nil
}
