	...
	return func(c *gin.Context) {
		...
		if ok {
			...
			if shouldSelectChannel {
				...
				// 智能调度：model 命中别名注册表（如 "auto"）时，在 (模型 × 渠道)
				// 候选并集上按 balanced 双维策略（效果×速度 + 字节生态倾斜）选定
				// 最终组合，写回 modelRequest.Model 后跳过常规选渠道流程。
				// 未命中/失败时 channel 保持 nil，自然落回原有 affinity/随机选择逻辑。
				if ch, resolvedModel, ok := routing.ResolveAutoModel(c, modelRequest.Model, usingGroup); ok {
					channel = ch
					modelRequest.Model = resolvedModel
					common.SysLog(fmt.Sprintf("[Distribute] auto-route: alias resolved -> model=%s channel=#%d", resolvedModel, ch.Id))
				}
				...
				if preferredChannelID, found := service.GetPreferredChannelByAffinity(c, modelRequest.Model, usingGroup); found && channel == nil {
