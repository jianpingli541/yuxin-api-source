...
## v1.2.20-yuxin (2026-08-18)

用户注册页验证校验加强：前后端规则对齐 + 密码正式策略上线。

### 注册校验（register-validation）
- 用户名：3-20 位，仅字母/数字/下划线/连字符（前端 zod regex + 后端 `common.ValidateUsername`，此前仅非空+max=20）
- 密码：8-20 位且须含大写字母+小写字母+数字+特殊符号（`common.ValidatePasswordStrength` 由测试期 >=6 位临时策略升级为正式策略；注册与改密两处调用同时生效）
- 邮箱：选填但填了必须合法格式（前端 zod union email + 后端 `net/mail` 解析，此前完全不验格式）
- 前端密码占位文案同步新策略；新增 3 条 i18n 键（en/zh/zh-TW，其余语言回退英文）
- 修复前后端策略不一致：前端 8-20 位 vs 后端 >=6 位，此前可绕过前端直打 API 注册弱密码账号
- 验证：go build + go test ./common PASS；bun run build PASS；注册接口 8 组非法输入全拒 + 合法输入全过（见发布记录）

## v1.2.18-yuxin (2026-08-18)

智能调度 P0 上线：客户端 model:"auto" 别名调度 + balanced 双维策略 + 字节生态倾斜。

### 智能调度（auto-route）
- 客户端 `model:"auto"` → 网关基于「效果(canary 质量分)×速度(实测延迟)」加权融合选定 (模型 × 渠道) 组合；`X-Yuxin-Route: balanced|quality|speed` 逐请求微调权重
- 字节跳动生态（VolcEngine/Doubao 渠道类型）候选得分 ×(1+doubao_boost)，默认 +15%——新模型上架进池即自动享受倾斜，客户优先尝鲜
- 计费按实际生效模型扣费（现有计费链零改动）；响应头 `X-Yuxin-Model` 返回实际模型、`X-Yuxin-Alias` 标记别名命中
- 配置：系统设置 → 模型与路由 →「智能调度」板块（总开关/别名池 JSON/倾斜系数），注册 GlobalConfig 自动持久化
- 架构：middleware/distributor.go 接入（token 模型限制校验后）；affinity 分支加 channel==nil 守卫防覆盖；relay 层 SmartSelectChannel 短路防二次打散；CORS 放行 X-Yuxin-Route 并暴露 X-Yuxin-Model/Alias
- 修复：SmartRouter.GetQualityScore 打通 canary 活数据（此前 UpdateQualityScore 无人调用，质量分恒为默认 75 空转）
- 测试：balanced 单测 5/5 PASS；E2E 实测 model:auto 三种策略头全 200 + X-Yuxin-Model 正确 + 计费按生效模型 + 未知别名/开关关闭正确回退 503
- 对标：OpenRouter openrouter/auto + LiteLLM Router 策略路由

### 前端（同日早些时候）
- 导航：顶部 Help 移除；侧边栏 Personal 组新增「使用指南」(/help)；管理员组 Admin Help 改名「管理指南」（i18n 补全，此前中文界面裸显英文）

## v1.2.14-yuxin (2026-08-17)

智能调度差距分析（2026-08-17-smart-routing-gap-analysis.md）P1 先行项 + 工程卫生。

### 路由透明化
- 响应新增 `X-Yuxin-Model` 头：暴露实际生效的上游模型名（模型映射/重试后取最终值），注入点 ModelMappedHelper（覆盖 chat/responses/gemini/embedding/rerank/task 等 8 个 handler）；刻意不暴露渠道信息（保护供应链）

### 工程卫生
- typecheck 27 → 23：补 `positiveIntegerSchema` 导出（zod）、login-session-utils.test 3 处过期 4 参调用修正、补 happy-dom devDep；剩余 23 个均为测试文件与演进 API 脱节，列入 backlog

### 运维配置（生产已生效，UI 实测回读）
- 跨渠道重试次数 0 → 2（此前单渠道故障=用户直接报错）
- 定期渠道测试开启（10 分钟全量）：渠道卡片「上次测试」已从 12 天前变为实时，系统任务可见 channel_test
- root 开启「接收上游模型更新通知」（新模型雷达，个人资料 → 设置与偏好）
- 发现：模型性能指标采集（延迟/成功率）此前已启用 → P1 延迟感知调度数据基础已具备


## v1.2.13-yuxin (2026-08-17)

帮助文档配套实测修复版（USER-HELP/ADMIN-HELP 编写过程中浏览器全流程实测发现）。

### 前端修复
- 注册密码校验 min(6) → min(8)：与占位符 "8-20 characters" 及 7 语言既有翻译对齐（此前提示与校验不一致）
- 协议勾选句硬编码英文 ' and the ' → t('and the')，7 locale 补齐（zh/zh-TW 和、ja と、fr ainsi que、ru и、vi và）：修复中英混排
- 新增 /chat 索引路由 → 302 重定向 /playground：修复无会话 ID 直访 /chat 落 404（C5-②）

### 文档
- docs/USER-HELP.md + docs/ADMIN-HELP.md：双角色使用帮助，全量浏览器实测（真实注册账号 + root 管理后台逐页），截图证据 30+

### 实测澄清
- C5-①「Sign out 无 logout API 调用」为过时记录：登出链路（下拉菜单 → 确认弹窗 → logout API → 会话失效）实测完整可用，关闭该项
