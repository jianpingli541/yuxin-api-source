...
export function Wallet(props: WalletProps) {
  ...
  const handlePaymentMethodSelect = async (method: PaymentMethod) => {
    ...
    try {
      ...
      // 注意：后端微信原生支付的 type 是 'wechat'（model.PaymentMethodWechat），
      // 历史聚合占位是 'wxpay'，两种都要兼容；支付宝必须走 processAlipayPayment，
      // 此前误用 processWechatPayment 导致支付宝按钮走微信下单。
      const isWechat =
        method.type === PAYMENT_TYPES.WECHAT || method.type === 'wechat'
      ...
      if (isWechat || isAlipay) {
        ...
        if (isWechat) {
          ...
          const result = await processAlipayPayment(topupAmount, 0)
          if (result) {
